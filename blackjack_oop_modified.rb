...
class Player
  ...
  attr_reader :name, :hand_total, :bet
...
#@@player_list = []
  ...
  def winner?
    Player::player_list.each do |player|
      if !@active_players.include?(player.name)
        next
      end
      if player.hand_total > @dealer.total.to_i || @dealer.busted?
      @active_players.select! {|name| name != player.name}
      puts "#{player.name} wins against dealer"
    elsif player.hand_total < @dealer.total.to_i
      @active_players.select! {|name| name != player.name}
      puts "Dealer wins against #{player.name}"
    elsif player.hand_total == @dealer.total.to_i
        puts "#{player.name} ties with Dealer"
        @active_players.select! {|name| name != player.name}
    else
      next
    end
  ...
  def hit_or_stay?
    choices = ['H','S']
    Player::player_list.each do |player|
        while !player.is_busted  && player.total.to_i != 21
  ...
  def blackjack?
    Player::player_list.each do |player|
      if player.blackjack && !dealer_blackjack?
        puts "#{player.name} hit 21!"
        player.cash += (player.bet * 1.5 + player.bet)
        @active_players.select! {|name| name != player.name}
      end
    end
  end
